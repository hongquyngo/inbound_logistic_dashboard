...
def get_uninvoiced_ans(filters: Dict = None) -> pd.DataFrame:
    """
    Get all ANs with uninvoiced quantity using can_invoice_view (lean view).
    Derived columns (estimated_invoice_value, legacy qty, true_remaining_qty)
    computed in pandas after fetch — no CTE / SUBSTRING_INDEX in SQL.
    """
    try:
        ...
        # ── Derived columns in pandas (zero extra DB round-trip) ─────────────
        ...
        # Legacy invoice quantities — one targeted query, merged in pandas
        ppo_ids = df["product_purchase_order_id"].dropna().unique().tolist()
        if ppo_ids:
            legacy_df = _get_legacy_invoice_qtys(tuple(sorted(ppo_ids)))
            df = df.merge(legacy_df, on="product_purchase_order_id", how="left")
            df["legacy_invoice_qty"]   = df["legacy_invoice_qty"].fillna(0)
            df["legacy_invoice_count"] = df["legacy_invoice_count"].fillna(0)
        else:
            df["legacy_invoice_qty"]   = 0
            df["legacy_invoice_count"] = 0

        df["has_legacy_invoices"] = df["legacy_invoice_qty"].apply(
            lambda x: "Y" if x > 0 else "N")

        df["true_remaining_qty"] = (
            df[["uninvoiced_quantity", "po_line_pending_invoiced_qty"]]
...
@st.cache_data(ttl=300)
def _get_legacy_invoice_qtys(ppo_ids_tuple: tuple) -> pd.DataFrame:
    """Legacy invoice quantities for given PO lines — cached independently."""
    try:
        engine = get_db_engine()
        query = text("""
            SELECT
            ...
            WHERE pid.arrival_detail_id IS NULL
              ...
              AND pid.product_purchase_order_id IN :ppo_ids
            GROUP BY pid.product_purchase_order_id
        """)
        with engine.connect() as conn:
            return pd.read_sql(query, conn, params={"ppo_ids": ppo_ids_tuple})
    except Exception as e:
        logger.error(f"Error fetching legacy invoice qtys: {e}")
        return pd.DataFrame(columns=["product_purchase_order_id",
...
def _empty_filter_options() -> Dict:
    return {k: [] for k in ['creators', 'vendor_types', 'vendors', 'entities',
...
def validate_invoice_selection(selected_df: pd.DataFrame) -> Tuple[bool, str]:
    ...
    # Check vendor type consistency (column renamed in lean view)
    vtype_col = 'vendor_type' if 'vendor_type' in selected_df.columns else 'vendor_location_type'
    if vtype_col in selected_df.columns:
        vendor_types = selected_df[vtype_col].unique()
        if len(vendor_types) > 1:
    ...
    # Check single legal entity (column renamed in lean view)
    entity_col = 'legal_entity_code' if 'legal_entity_code' in selected_df.columns else 'consignee_code'
    if entity_col in selected_df.columns:
        entities = selected_df[entity_col].unique()
        if len(entities) > 1:
    ...
    try:
        with engine.begin() as conn:
            ...
            header_params = {
                ...
                # usd_exchange_rate is NOT NULL — always provide a safe default
                'usd_exchange_rate': invoice_data.get('usd_exchange_rate') or 1.0,
            ...
            for _, row in details_df.iterrows():
                ...
                # VAT is already in details_df (fetched by get_invoice_details)
...
def generate_invoice_number(vendor_id: int, buyer_id: int, is_advance_payment: bool = False) -> str:
    ...
    Uses AUTO_INCREMENT from information_schema (race-condition safe) instead of MAX(id)+1.
    Falls back to microsecond timestamp if query fails.
    """
    today = datetime.now()
    ...
    try:
        engine = get_db_engine()
        # AUTO_INCREMENT reflects the *next* id that will be assigned — no race condition
        query = text("""
        ...
        with engine.connect() as conn:
            ...
            next_id = int(result[0]) if result and result[0] else 1

        return f"V-INV{date_str}-{vendor_id}{buyer_id}{next_id}-{suffix}"

    except Exception as e:
        logger.error(f"Error generating invoice number: {e}")
        # Fallback: microsecond timestamp guarantees uniqueness in practice
        ts = today.strftime('%H%M%S%f')[:10]
        return f"V-INV{date_str}-{vendor_id}{buyer_id}{ts}-{suffix}"
